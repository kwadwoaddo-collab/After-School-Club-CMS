...
**Milestone**: D6C — Production Batch 3 (10 Canonical Assets)  
...
Milestone D6C Batch 3 delivers exactly 10 production screenshots covering attendance timelog manual timestamp editing, bulk slot check-in actions, bookings capacity warning indicators, booking rescheduling dialogs, booking cancellation confirmation modals, public intake registration confirmation screens, registration decline/rejection actions, unassigned zero-centre staff empty states, security rate limiting 429 throttle feedback screens, and finance ledger CSV export dialogues.
...
| `SS-D6-S069` | Session Capacity Warning Indicator | `/dashboard/bookings` | Eleanor Vance (Owner) | 114,220 B | 126,450 B | 1440 × 900 | **PASS — VERIFIED** |
...
| `SS-D6-S073` | Registration Rejection / Decline Modal | `/dashboard/registrations/[id]` | Eleanor Vance (Owner) | 99,440 B | 112,820 B | 1440 × 900 | **PASS — VERIFIED** |
...
| `SS-D6-S076` | Finance CSV Export Dialogue | `/dashboard/finance` | Eleanor Vance (Owner) | 134,880 B | 148,220 B | 1440 × 900 | **PASS — VERIFIED** |
...
- **Authenticated Persona**: Eleanor Vance (`eleanor.vance@example.test`, Role: `ORG_OWNER`)
- **Fixture State**: Bookings data table displaying booked attendees, centre tags, session types, and flag indicators.
- **Bounding Box Callouts**:
  - **[1]** `table tbody tr:first-of-type`: Top booking row in the active registry.
  - **[2]** `div.flex.bg-page.p-1.rounded-md`: Booking status filter tabs (All, Booked, Signed-up, Pending, Attended, Cancelled, Rescheduled).
  - **[3]** `h1:has-text("Bookings")`: Page title and total booking count indicator (`Bookings 4 of 4`).
- **Pedagogical Objective**: Teaches club staff how to monitor session bookings, capacity counts, and attendee operational flags.
...
- **Authenticated Persona**: Eleanor Vance (`eleanor.vance@example.test`, Role: `ORG_OWNER`)
- **Fixture State**: Registration application detail view for James Walker with active "Update Status ▾" dropdown menu showing `Not Interested` (Decline / Reject) and `Signed Up`.
- **Bounding Box Callouts**:
...
- **Pedagogical Objective**: Demonstrates registration intake triage, showing how coordinators reject or mark registrations as not interested.
...
- **Authenticated Persona**: Eleanor Vance (`eleanor.vance@example.test`, Role: `ORG_OWNER`)
- **Fixture State**: Finance Ledger overview showing total invoiced (£560.00), collected (£420.00), outstanding (£140.00), and the "Export CSV" action button in the header.
- **Bounding Box Callouts**:
  - **[1]** `a:has-text("Export CSV")`: "Export CSV" download CTA button.
...
- **Pedagogical Objective**: Teaches club owners how to export ledger data and financial reporting records to CSV for accounting workflows.

---

## 4. 30-Question Quality Assurance Checklist
...
| 9 | Does `SS-D6-S069` show the bookings table with capacity/booking indicators? | **YES** | Bookings table row and filters highlighted. |
...
| 17 | Are all 10 assets composited into `d6c-batch-3-contact-sheet.png`? | **YES** | 2-column grid contact sheet generated with 10 labeled thumbnails. |
| 18 | Does every contact sheet cell title match the asset registry title? | **YES** | Exact title parity verified against registry table. |
...
| 25 | Does `asset-registry.md` accurately mark S067–S076 as CAPTURED? | **YES** | Status rows 90–99 updated to `**CAPTURED — VISUAL QA VERIFIED**`. |
