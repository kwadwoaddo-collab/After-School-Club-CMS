...
## Part 3: The Daily Attendance, Classroom Delivery & Safeguarding Journey

---

## 1. Overview of the Daily Session Journey

This master journey maps the operational and safety lifecycle of a student during a live club session — from morning/afternoon arrival through classroom roll call, operational flags, departure pickups, first aid handling, and confidential safeguarding escalation.
...
│  Tutor/Front Desk taps "Check In" (Custodial timestamp set) │
...
└──────────────────────────────┬──────────────────────────────┘
   ...
   Staff applies cold compress;     Tutor listens calmly; makes zero
   Front Desk logs Accident with    public notes; escalates in person
   treatment & signature.           to DSL; DSL logs in `/incidents`.
               │                               │
               └───────────────┬───────────────┘
                               │
┌──────────────────────────────▼──────────────────────────────┐
...
│  Taps "Check Out" (Departure timestamp legally recorded)    │
└─────────────────────────────────────────────────────────────┘
```
...
- Staff review the expected headcount to ensure adequate staff-to-child ratios.
- **Medical Alert Badges** (Red = Severe Allergy/Condition, Yellow = Dietary, Blue = SEN) are reviewed during the pre-session team briefing.

---

## 3. Stage 2: Physical Arrival & Statutory Check-In
...
During the session:
- **Homework & Behaviour:** Tutors tap the flag icons on the attendance register to record completed homework or exemplary positive conduct.
...
SprintScale bifurcates safety events into two strict operational pathways:

### Pathway A: Standard First Aid & Minor Accidents
- **Examples:** Playground tumble, skinned knee, paper cut, scheduled asthma inhaler administration.
...
- **Visibility:** Visible to Front Desk, Managers, and Owners.

### Pathway B: Confidential Child Safeguarding Concerns
- **Examples:** Physical injury marks, signs of abuse/neglect, explicit verbal disclosures.
- **Workflow:** Tutor listens without judgment; provides immediate physical safety; **records zero notes in general student files**; reports verbally in private to the Centre Manager (Designated Safeguarding Lead).
- **CMS Recording:** The Centre Manager authors a confidential report in `Sidebar → Incidents` under the `Safeguarding` type.
- **Visibility:** Strictly restricted to Managers and Owners acting as DSLs.

---

## 6. Stage 5: Verified Pickup & Custodial Check-Out
...
5. The `checkOutAt` departure timestamp is recorded, completing the statutory custodial audit loop.
