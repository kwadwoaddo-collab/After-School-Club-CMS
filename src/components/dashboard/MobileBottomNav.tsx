...
export default function MobileBottomNav({ userRole = 'TUTOR' }: MobileNavProps) {
    ...
    return (
        <nav
            className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#0f172a] border-t border-white/10 mobile-bottom-nav"
                ...
                {items.map(item => {
                    ...
                    return (
                        <Link
                            ...
                            className={`relative flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all min-h-[44px] min-w-[44px] justify-center ${isActive
                                    ? 'text-white'
                                    : 'text-slate-500 hover:text-slate-300'
                                }`}
                        >
                            {/* Active top indicator - needs `relative` on parent to work */}
                            {isActive && (
