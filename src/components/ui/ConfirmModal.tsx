...
export default function ConfirmModal({
    ...
    const iconColor = variant === 'danger' ? 'text-error' : variant === 'warning' ? 'text-amber-500' : 'text-blue-500';
    const iconBg = variant === 'danger' ? 'bg-error/10 border-error/20' : variant === 'warning' ? 'bg-amber-500/10 border-amber-500/20' : 'bg-blue-500/10 border-blue-500/20';
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-surface-container-high border border-outline-variant/20 rounded-3xl shadow-2xl p-8 max-w-sm w-full mx-4 animate-in zoom-in-95 duration-200 relative">
                ...
                <div className="flex gap-3 w-full">
                    <button
                        onClick={onClose}
                        className="flex-1 bg-surface-container-highest text-white font-bold py-3.5 rounded-xl hover:bg-surface-container-highest/80 transition-all border border-outline-variant/10"
                    >
                        {cancelText}
                    </button>
                    <button
