...
import { eq, sql, and, gte, lt, lte, inArray, or, asc } from 'drizzle-orm';
...
import { ArrowRight, ChevronRight, AlertTriangle, Shield, ClipboardList, CalendarCheck, ArrowUpRight, ArrowDownRight, Minus } from 'lucide-react';
import { startOfWeek, endOfWeek, startOfMonth, endOfMonth, parseISO, format, subWeeks, subMonths } from 'date-fns';
import { resolveAttendanceStatus, getAttendanceColorClass, AttendanceStatus } from '@/lib/attendance';
...
export default async function ActivityTab({ searchParams, org, activeCentreId, accessibleCentreIds, hasCentres }: ActivityTabProps) {
    ...
    const safetyNotes = recentBookingsChildIds.length > 0
    ...
    const calculateTrend = (curr: any, prev: any) => {
        ...
        return { diff: current - previous, text: perc > 0 ? `+${perc.toFixed(0)}%` : `${perc.toFixed(0)}%`, type: typeStr as "positive" | "negative" | "neutral" };
    };

    const bookingsTrend = calculateTrend(Number(bookingKpis.activePeriod), Number(bookingKpis.prevPeriod));
    const totalRegistrations = Number(registrationKpis.total);
    const bookingsThisWeek = Number(bookingKpis.thisWeek);
        ...
        <div className="space-y-6 animate-in fade-in duration-500 slide-in-from-bottom-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className={cn(
                    ...
                    <div className="flex flex-col items-center gap-0 relative z-10 w-full">
                        ...
                        <div className="flex flex-col items-center py-1.5 gap-1">
                            ...
                            <span className="px-2.5 py-0.5 rounded-full bg-secondary border border-border text-[10px] font-black text-muted-foreground tabular-nums">
                                ↓ {totalRegistrations > 0 ? Math.round(((pipelineCounts.new + pipelineCounts.approved) / totalRegistrations) * 100) : 0}% active
                            </span>
                            <div className="w-px h-2 bg-border" />
                        </div>
                        ...
                        <div className="w-[88%]">
                            <div className={cn(
                                ...
                                pipelineCounts.new > 0 ? "bg-amber-500/10 border-amber-500/30" : "bg-secondary border-border"
                            )}>
                                <div className="flex items-end justify-between">
                                    <div>
                                        <p className={cn(
                                            "text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5",
                                            pipelineCounts.new > 0 ? "text-amber-700 dark:text-amber-400" : "text-muted-foreground"
                                        )}>
                                            Pending Review
                                            {pipelineCounts.new > 0 && (
                                                <span className="relative flex h-1.5 w-1.5">
                                        ...
                                        <p className={cn(
                                            ...
                                            pipelineCounts.new > 0 ? "text-amber-700 dark:text-amber-400" : "text-foreground"
                                        )}>
                                            {pipelineCounts.new}
                                        </p>
                                    </div>
                    ...
                    {pipelineCounts.new > 0 ? (
                        <Link href="/dashboard/registrations?status=awaiting_confirmation" className="mt-auto flex items-center justify-center gap-2 py-2.5 rounded-full bg-amber-500/10 text-amber-700 dark:text-amber-400 text-sm font-semibold hover:bg-amber-500/20 transition-colors border border-amber-500/20 relative z-10 group/btn">
                            Review {pipelineCounts.new} Pending
                            <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform" />
                        </Link>
                    ) : (
                        <Link href="/dashboard/registrations" className="mt-auto flex items-center justify-center gap-2 py-2.5 rounded-full bg-secondary text-muted-foreground text-sm font-semibold hover:bg-secondary/80 transition-colors border border-border relative z-10 group/btn">
                            View Pipeline
                            <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1 transition-transform" />
                        </Link>
                    )}
                    ...
                    <div className="flex items-end justify-between relative z-10 p-4 rounded-xl bg-secondary/40 border border-border">
                        ...
                        <div className="text-right">
                            {bookingsTrend.type !== 'neutral' && (
                                <div className={cn("flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold border", bookingsTrend.type === 'positive' ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20" : "bg-error/10 text-error border-error/20")}>
                                    {bookingsTrend.type === 'positive' ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />}
                        ...
                        {recentBookingsWithNotes.length > 0 ? (
                            <div className="space-y-2">
                                {recentBookingsWithNotes.map((b: any) => (
                                    <Link key={b.id} href={`/dashboard/bookings/${b.id}`} className="flex items-center justify-between p-3 rounded-xl bg-card hover:bg-secondary/60 border border-border transition-colors duration-150 active:scale-[0.99] cursor-pointer group">
                                        <div className="flex items-center gap-3">
                                            <AttendanceRadial percentage={b.attendanceStats.total > 0 ? (b.attendanceStats.completed / b.attendanceStats.total) * 100 : 0} size="sm" title={b.attendanceStats.total === 0 ? "No attendance data yet" : `${b.attendanceStats.completed}/${b.attendanceStats.total} sessions attended`}>
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {(() => {
                                                        const resolved = resolveAttendanceStatus((b.attendanceStatus as AttendanceStatus | null) ?? null, b.status);
                                                        return (
                                                            <span className={cn("px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider border", getAttendanceColorClass(resolved.status), resolved.status === 'pending' ? 'bg-secondary text-muted-foreground border-border' : 'border-current/20')}>
                                                                {resolved.label}
                                                            </span>
                                                        );
                                                    })()}
                            ...
                            <div className="flex flex-col items-center gap-3 py-10 text-center">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-muted-foreground">No bookings for this period</p>
                                    <p className="text-xs text-muted-foreground/60 mt-1">Schedule a session to see it here</p>
                                </div>
                                <Link href="/dashboard/bookings/new" className="mt-1 text-xs font-semibold text-primary hover:underline">Create a booking →</Link>
                            </div>
                        )}
                    </div>

                    <Link href="/dashboard/bookings" className="mt-auto flex items-center justify-center gap-2 py-2.5 rounded-full bg-secondary text-muted-foreground text-sm font-semibold hover:bg-secondary/80 transition-colors border border-border relative z-10 group/btn">
                ...
                <div className={cn("glassmorphic-card p-6 rounded-3xl border border-border relative overflow-hidden", "hover:border-primary/30 transition-all duration-300 flex flex-col gap-6")}>
                    ...
                    <div className="flex items-center justify-between relative z-10">
                        ...
                        <span className={cn("px-2.5 py-1 rounded-full text-[10px] font-black tabular-nums border", pipelineCounts.new > 0 ? "bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20" : "bg-secondary text-muted-foreground border-border")}>
                        ...
                        {recentRegistrations.length > 0 ? (
                            <div className="space-y-2">
                                {recentRegistrations.map((r: any, i: any) => (
                                    <Link key={`${r.registrationId}-${i}`} href={`/dashboard/registrations/${r.registrationId}`} className="flex items-center justify-between p-3 rounded-xl bg-card hover:bg-secondary/60 border border-border transition-colors duration-150 active:scale-[0.99] cursor-pointer group">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    <span className={cn("px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider border", r.status === 'awaiting_confirmation' ? "bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20" : r.status === 'signed_up' ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20" : "bg-secondary text-muted-foreground border-border")}>
                                                        {r.status === 'awaiting_confirmation' ? 'Pending' : r.status === 'signed_up' ? 'Approved' : 'Other'}
                                                    </span>
                                                </div>
                                                <p className="text-xs text-muted-foreground font-medium mt-0.5">
                            ...
                            <div className="flex flex-col items-center gap-3 py-10 text-center">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-muted-foreground">No registrations for this period</p>
                                    <p className="text-xs text-muted-foreground/60 mt-1">Share your registration link to get started</p>
                                </div>
                                <Link href={registrationLink} className="mt-1 text-xs font-semibold text-primary hover:underline">Open registration form →</Link>
                            </div>
                        )}
                    </div>
                    ...
                    <Link href="/dashboard/registrations" className="mt-auto flex items-center justify-center gap-2 py-2.5 rounded-full bg-primary/10 text-primary text-sm font-semibold hover:bg-primary/20 transition-colors border border-primary/20 relative z-10 group/btn">
