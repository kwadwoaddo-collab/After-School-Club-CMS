...
export default function ParentProfileClient({ parent, invoices, stats, isOwner }: ParentProfileClientProps) {
    ...
    return (
        <div className="space-y-6">
            {/* Tabs */}
            <div className="flex items-center gap-2 p-1 bg-surface-container-high rounded-2xl w-fit border border-outline-variant/10">
                <button 
                        ...
                        <div className="glassmorphic-card rounded-[40px] p-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {parent.children?.map((child: any) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-xl bg-surface-container-low flex items-center justify-center text-emerald-400">
                    ...
                    <div className="space-y-6">
                        <div className="bg-primary/10 border border-primary/20 rounded-[40px] p-8 text-center relative overflow-hidden ring-1 ring-primary/30">
                            <div className="relative z-10">
                                ...
                                <h2 className={`text-4xl font-black ${stats.outstanding > 0 ? 'text-error' : 'text-emerald-400'}`}>
            ...
            {activeTab === 'finance' && (
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className={`p-6 border rounded-3xl flex items-center justify-between ${stats.outstanding > 0 ? 'bg-error/5 border-error/20' : 'bg-emerald-500/5 border-emerald-500/20'}`}>
                            <div>
                                <p className={`text-[10px] font-black uppercase tracking-widest ${stats.outstanding > 0 ? 'text-error' : 'text-emerald-400'}`}>Amount Due</p>
                                <p className={`text-xl font-black mt-1 ${stats.outstanding > 0 ? 'text-error' : 'text-emerald-400'}`}>£{stats.outstanding.toFixed(2)}</p>
                            </div>
                            <AlertCircle className={`w-8 h-8 ${stats.outstanding > 0 ? 'text-error/40' : 'text-emerald-400/40'}`} />
                        </div>
                    </div>
