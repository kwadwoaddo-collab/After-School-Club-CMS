...
function AttendeeAssessment({ attendee }: { attendee: BookingWithDetails['attendees'][0] }) {
    ...
    const [isSaving, startSaving] = useTransition();
            ...
            <div className="space-y-4">
                {/* Score Input */}
                <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-1">Score (Optional)</label>
                    <input
                        type="text"
                        value={score}
                        ...
                        className="w-full bg-background border-border text-foreground rounded-md px-3 py-1.5 text-sm focus:ring-1 focus:ring-primary disabled:opacity-50 placeholder:text-muted-foreground"
                    />
                </div>

                {/* Notes Input */}
                <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-1">Club Leader Notes</label>
                    <textarea
                        value={notes}
                ...
                {/* File Upload */}
                <div>
                    <label className="block text-xs font-medium text-muted-foreground mb-1">Session Activity / Worksheet (Photo)</label>
                        ...
                        status !== 'SENT' && (
                            <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-700 border-dashed rounded-md hover:border-indigo-500/50 transition-colors group cursor-pointer relative">
                                <div className="space-y-1 text-center">
                                    <svg className="mx-auto h-8 w-8 text-muted-foreground group-hover:text-primary transition-colors" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
                ...
                {status !== 'SENT' && (
                    <div className="flex items-center gap-2 justify-end pt-2 border-t border-slate-700/50 mt-2">
                        <button
                            ...
                            className="px-3 py-1.5 text-xs font-medium text-primary hover:text-primary hover:bg-primary/10 rounded border border-primary/50 transition-colors disabled:opacity-50"
                        >
                        ...
                        {sendConfirm ? (
                            <>
                                <span className="text-xs text-muted-foreground">Send to parent?</span>
                                <div className="flex items-center gap-2">
                                    <button
                                        onClick={() => setSendConfirm(false)}
                                        className="px-3 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground rounded border border-border transition-colors"
                                    >
                                        Cancel
                                    </button>
                                    <button
                                        ...
                                        className="px-3 py-1.5 text-xs font-bold text-primary-foreground bg-success hover:bg-success/90 rounded shadow-sm transition-all disabled:opacity-50"
                                    >
                            ...
                            <button
                                ...
                                className="px-3 py-1.5 text-xs font-bold text-primary-foreground bg-primary hover:bg-primary/90 rounded shadow-sm shadow-primary/20 transition-all disabled:opacity-50"
                            >
                                Send to Parent
                            </button>
                        )}
