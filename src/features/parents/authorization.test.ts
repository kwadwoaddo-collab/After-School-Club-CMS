 ...
 * the previously-untested PATCH endpoint, and documents the deliberate
 * decision to leave GET unrestricted (consumed cross-module by
 * BookingForm — see the audit doc).
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// 4. GET /api/parents/[id] — deliberately unrestricted (documents the decision)
// ─────────────────────────────────────────────────────────────────────────────

describe('GET /api/parents/[id] — deliberately left unrestricted', () => {
  beforeEach(() => {
    vi.resetModules();
    vi.clearAllMocks();
  });

  it('does not deny TUTOR — this endpoint is consumed cross-module by BookingForm, see project-notes/milestone-3b-parents-audit.md §4', async () => {
    const { auth } = await import('@/lib/auth');
    (auth as any).mockResolvedValueOnce(sessionFor('TUTOR'));
    const { db } = await import('@/db');
    (db.query.parents.findFirst as any).mockResolvedValueOnce(undefined);
    ...
    // 404 (not found, since the mock returns nothing) proves TUTOR reached
    // the DB lookup rather than being denied for their role.
    expect(res.status).toBe(404);
  });

  it('still denies an unauthenticated request with 401', async () => {
    const { auth } = await import('@/lib/auth');
    (auth as any).mockResolvedValueOnce(null);
    const { GET } = await import('@/app/api/parents/[id]/route');
