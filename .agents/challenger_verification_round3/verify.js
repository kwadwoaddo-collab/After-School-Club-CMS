...
const tscRes = spawnSync(process.execPath, [path.resolve(__dirname, '../../node_modules/typescript/bin/tsc'), '--noEmit'], {
    encoding: 'utf-8',
...
const eslintRes = spawnSync(process.execPath, [path.resolve(__dirname, '../../node_modules/eslint/bin/eslint.js')], {
    encoding: 'utf-8',
    env: { ...process.env, NODE_ENV: 'test' }
});
...
const vitestRes = spawnSync(process.execPath, [path.resolve(__dirname, '../../node_modules/vitest/vitest.mjs'), 'run'], {
    encoding: 'utf-8',
    env: { ...process.env, NODE_ENV: 'test' }
});
...
process.exit(tscRes.status || eslintRes.status || vitestRes.status || 0);
